	...
	$(transaksiTable).navButtonAdd(transaksiPager, {
   	caption: "All Reports",
		...
		onClickButton:function(){
			...
			rangeReport(totalRecord,formatExport);
		},
	})
...
function addtransaksi() {
	$(transaksiDialog).load(`${baseUrl}/data/create`, function() {
	...
	$.ajax({
		...
		success: function(res) {
			if (res.status == 'submitted') {
				...
				$.ajax({
					url:`${baseUrl}/api/data/${res.postData.nofaktur}?sort_index=${$(transaksiTable).jqGrid('getGridParam', 'sortname')}&postData=${res.postData[$(transaksiTable).jqGrid('getGridParam', 'sortname')]}&sort_order=${$(transaksiTable).jqGrid('getGridParam', 'sortorder')}&limit=${$(transaksiTable).jqGrid('getGridParam', 'postData').rows}`,
					type: 'GET',
					dataType: 'JSON',
					success: function(res) {
						if (res.data) {
...
function edittransaksi(nofaktur) {
	$(transaksiDialog).load(`${baseUrl}/data/edit/${nofaktur}`, function() {
		$($(transaksiForm + ' input')[0]).focus()
	}).dialog({
		modal: true,
...
function confirmDeletetransaksi(nofaktur) {
  $(transaksiDialog).load(`${baseUrl}/data/delete/${nofaktur}`)
		.dialog({
		modal: true,
	...
	function deletetransaksi(nofaktur) {
		$.ajax({
			url: `${baseUrl}/api/data/${nofaktur}/delete`,
			type: 'POST',
			dataType: 'JSON',
			success: function(res) {
				if (res == 'deleted') {
  ...
  $(transaksiDialog).load(`${baseUrl}/data/export/${nofaktur}?total=${totalRecord}`)
	...
	function exportConfirm(formatExport,data) {
		let params
			...
			window.open(`${baseUrl}/data/export?${params}`)			
	}
}
	...
	$(transaksiDialog).load(`${baseUrl}/data/report/${nofaktur}?total=${totalRecord}`)
	  ...
	  function reportConfirm(data) {
		  let params
			  ...
			  params +='&from='+ from;
			  params +='&to='+data[1]['value'];
			  window.open(`${baseUrl}/data/report?${params}`)
